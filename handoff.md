...
**✅ `supabase-content-queue-upgrade.sql` has been run.** **⚠️ `supabase-content-queue-pilot-ai-analysis.sql` has not been run yet** — execute it in the Supabase SQL Editor to actually attach the analysis below to the 7 pilot rows.
...
6. **`content`** (🗂️ ניהול תוכן) — approval workflow only, see §7 for the full pipeline:
   ...
   - **"👁 תצוגה"** opens a preview/edit modal (`#cqModal`) — **NEW, this session: FB/Instagram mockup + stored AI analysis**, then the editable fields, then actions: **"💾 שמירת שינויים"** (save edits, stay pending), **"✅ אשר ותזמן"** (save edits + approve in one step), **"✋ דחה"** (reject).
     - **Mockup tabs** (`.cq-mock-tab`, default: Facebook): two stylized "phone card" mocks — `.cq-mock-fb` (avatar + page name + timestamp row, caption, image, אהבתי/תגובה/שיתוף action row) and `.cq-mock-ig` (avatar + `mr.polish.floors` username, square image, heart/comment/share/save icon row, `username + caption` line). Pure CSS/HTML populated from the row's own `caption`/`media_url` in `openPreview()` — no new data, no API calls. Captions are truncated per-platform (`truncateForMock`, 220 chars for the FB mock / 125 for IG) to mimic each platform's real caption-collapsing behavior.
     - **AI Analysis panel** (`#cqAiSection`) — shown only when at least one of `ai_reach_forecast`/`ai_why_it_works`/`ai_brand_contribution` is set on the row; hidden entirely otherwise (verified both states). Always shows the disclaimer "אומדן כיווני מבוסס על ביצועי העמוד עד כה - לא תחזית מדויקת" — **do not remove this line if editing the section**; the whole point is these are authored qualitative estimates, not a real forecasting model, and the copy should never imply otherwise.
...
**✅ Done:** `supabase-business-expenses.sql`, `supabase-content-queue.sql`, `supabase-content-queue-pilot-seed.sql`, and `supabase-content-queue-upgrade.sql` have all been run by Ori. `SUPABASE_SERVICE_ROLE_KEY` and `CRON_SECRET` are set in Vercel. The Meta token saga (see §6) resolved with a genuine long-lived, non-expiring **Page** token (`debug_token` confirmed `"type": "PAGE"`, `"expires_at": 0`) — both `META_ACCESS_TOKEN` (long-lived User token, exchanged via the new `tools/exchange-long-lived-token.js`) and `META_PAGE_ACCESS_TOKEN` (derived from it) are current and verified working in production. The old meetings calendar has been fully replaced by "לוח תוכן" (§3, item 7), deployed and verified live. The FB/Instagram mockup preview and the AI Analysis panel are built into `#cqModal` (§3, item 6) and tested with mocked data — both states (analysis present / absent) confirmed working.

**⚠️ Still outstanding:**
- **Run `supabase-content-queue-pilot-ai-analysis.sql`** in the Supabase SQL Editor — attaches the authored AI analysis to the 7 pilot rows (by `title` match). Without this, the modal's AI Analysis section stays hidden for all of them (the code correctly hides it when the fields are empty — nothing broken, just nothing to show yet).
- **Go review the 7 pilot posts in "ניהול תוכן" (or "לוח תוכן") and approve/reject each one** — the first is scheduled 2026-07-12; nothing publishes until approved, by design.
